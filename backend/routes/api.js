...
router.get("/getRecentlyAdded", async (req, res) => {
  try {
    ...
    let recentlyAddedFromjellystics = await API.getRecentlyAdded({ libraryid: libraryid });

    let recentlyAddedFromjellysticsMapped = recentlyAddedFromjellystics.map((item) => {
      return {
        Name: item.Name,
    ...
    if (libraryid !== undefined) {
      ...
      if (lastSynctedItemDate !== undefined) {
        recentlyAddedFromjellysticsMapped = recentlyAddedFromjellysticsMapped.filter((item) =>
      ...
      const recentlyAdded = [...recentlyAddedFromjellysticsMapped, ...filteredDbRows];
    ...
    let recentlyAdded = [...recentlyAddedFromjellysticsMapped, ...filteredDbRows];
