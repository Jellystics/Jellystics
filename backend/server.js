...
try {
  createdb.createDatabase().then((result) => {
    if (result) {
      console.log("[JELLYSTICS] Database created");
    } else {
      console.log("[JELLYSTICS] Database exists. Skipping creation");
    }
    ...
    db.migrate.latest().then(() => {
      ...
      server.listen(PORT, LISTEN_IP, async () => {
        console.log(`[JELLYSTICS] Server listening on http://${LISTEN_IP}:${PORT}`);
  ...
  console.log("[JELLYSTICS] An error has occured on startup: " + error);
}
